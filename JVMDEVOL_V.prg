...
If ::oArc:SECUENCIA > 0 .AND. oApl:oNit:CODIGO # 2 .AND.;
 ...
 //aCta[2,6] := ::oArc:TOTALFLE
   aCta[3,7] := ::oArc:TOTALIVA
   aCta[3,9] := ::aDF[1] * 100
   aCta[4,6] := ::oArc:TOTALFAC - ::oArc:TOTALRET - ::oArc:TOTALICA
   aCta[5,6] := ::oArc:TOTALRET
   aCta[5,9] := ::aDF[3] * 100
   aCta[6,6] := ::oArc:TOTALICA
   aCta[6,9] := ::aDF[4] * 1000
   ...
   FOR nE := 1 TO LEN( aCta )
      If aCta[nE,6] > 0 .OR. aCta[nE,7] > 0
         ...
         FOR nK := 1 TO 4
            ...
            do case
            Case cSql == "BASE"
               aCta[nE,nK+1] := LTRIM(STR(nB,10,0))
            Case cSql == "COD-VAR"
